...
import * as ldaptsModule from "ldapts";

const { PORT, JWT_KEY } = process.env;

// JWT_KEY es OBLIGATORIA en todos los entornos: no existe fallback ni clave por
// defecto. Si falta, configHealth lo loguea y el servidor arranca en modo
// degradado (la raíz / muestra el error y la API responde 503).
...
$_RETURN_DATA_ = addresses;
    ...
    ldap: {
      fn: request && reply ? ldaptsModule : undefined,
      description:
      ...
      web: "https://ldapts.js.org/",
      params: [
        {
          name: "new Client({ url, tlsOptions? })",
          description:
            "Creates an LDAP client. `url` uses the `ldap://` or `ldaps://` scheme, e.g. `ldaps://dc.example.com:636`. `tlsOptions` is optional and only applies when the URL uses `ldaps://`.",
          required: false,
          type: "function",
        },
        {
          name: "client.bind(userDN, password, controls?)",
          description:
            "Authenticates against the directory. `userDN` is the full distinguished name (e.g. `CN=svc-api,OU=Service Accounts,DC=example,DC=com`) or a UPN such as `user@example.com`. On failure it rejects with an LDAP-specific error.",
          required: false,
          type: "function",
        },
        {
          name: "client.search(baseDN, { scope?, filter?, attributes?, sizeLimit?, timeLimit?, paged? })",
          description:
            "Searches entries below `baseDN`. Returns `{ searchEntries, searchReferences }`. `scope` is `base`, `one`, or `sub`; `filter` is an LDAP filter string; `attributes` limits returned attributes; `sizeLimit` caps results; `paged: true` requests paged results.",
          required: false,
          type: "function",
        },
        {
          name: "client.add(dn, attributes) / client.modify(dn, changes) / client.del(dn)",
          description:
            "Writes entries. `modify` receives an array of `Change` objects (e.g. `new ldap.Change({ operation: 'replace', modification: new ldap.Attribute({ type: 'mail', values: ['new@example.com'] }) })`).",
          required: false,
          type: "function",
        },
        {
          name: "client.unbind()",
          description:
            "Gracefully closes the connection. Always call it in a `finally` block after bind/search work is done.",
          required: false,
          type: "function",
        },
        {
      ...
      return: {
        type: "object",
        description:
          "ldapts module exposing the Client class, filter constructors (AndFilter, OrFilter, EqualityFilter, etc.), Attribute, Change, and error classes.",
        object: [
          {
            name: "Client",
            type: "class",
            description: "LDAP client instance built with `new Client({ url })`.",
          },
          {
            name: "Attribute",
            type: "class",
            description: "Attribute definition used in add/modify operations.",
          },
          {
            name: "Change",
            type: "class",
