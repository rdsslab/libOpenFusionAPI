...
3. **Internal API Calls (`uFetchAutoEnv` / `uFetch`)**:
   ...
   - Use `uFetchAutoEnv.auto('/api/endpoint/path/auto')` to isolate environments. Clarification: `uFetchAutoEnv` receives a relative path, and you must replace the environment suffix (`dev`, `qa`, or `prd`) at the end of the route path with `auto`. This allows `uFetchAutoEnv` to internally detect and substitute the appropriate environment prefix of the caller at runtime.
   - **`auto()` takes exactly one argument.** The two-argument form is `create(url, shouldApplyAuto)`; `auto(url)` is just a shortcut for `create(url, true)`. A second argument passed to `auto()` is **silently ignored** — it neither fails nor changes behaviour, so code like `auto(path, true)` looks intentional while doing nothing.
   - **CRITICAL — Authorization is NOT forwarded automatically.** The only header the runtime propagates to the returned instance is `ofapi-trace-id`. If the target endpoint requires authentication you must pass it yourself, or the call fails with 401/403 for a reason that appears nowhere in the code: `await uF.get({ data, headers: { authorization: request.headers.authorization } })`.
