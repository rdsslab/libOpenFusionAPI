...
- Recurring tasks: interval tasks are not endpoints either. See [interval_tasks/HUMAN_GUIDE.md](interval_tasks/HUMAN_GUIDE.md) for operators, [interval_tasks/README.md](interval_tasks/README.md) for the `ofapi_intervaltask` model and runtime, and [interval_tasks/AI_SKILL.md](interval_tasks/AI_SKILL.md) for the agent contract served by `get_interval_task_skill`. A task schedules an existing endpoint on a fixed interval or a cron expression with timezone and execution window, and is managed with `list_interval_tasks` / `get_interval_task_runs` (read) and `upsert_interval_task` / `run_interval_task_now` / `reset_interval_task_attempts` / `delete_interval_task` (write).
- Auth & users: see [auth/USER_RECOVERY.md](auth/USER_RECOVERY.md) for the user lifecycle, self-service password change, admin password reset, and the OTP password-recovery flow (email/Telegram) including AppVars, the Telegram recovery bot and cleanup task.
- System parametrization (human reference): server-side configuration of the `system` app (AppVars, internal endpoints, interval tasks), including the admin alerts delivered to Telegram. See [system/README.md](system/README.md). This section is intentionally not exposed to AI agents — it is not a reusable bot/provider skill.
- Internal flow diagrams (human reference, Mermaid): see [flows/README.md](flows/README.md). This section is intentionally not exposed to AI agents — agents rely on the `AI_SKILL.md` contracts instead (see [DOCUMENTATION_SYSTEM_REPORT.md](DOCUMENTATION_SYSTEM_REPORT.md)).
